...
- **API 키 보관 방식** — 로그인에 사용한 키는 서버 세션 메모리에만 두며 DB·파일에 저장하지 않습니다. 서버가 재시작되면 사라집니다.
...
def render_login_page():
    ...
    with center:
        st.title("🔐 로그인")
        st.caption("OpenAI API 키로 로그인합니다.")
        ...
        server_key, source = resolve_api_key()

        if server_key:
            st.success(f"서버에 등록된 키가 있습니다 · 출처: {source}")
            st.code(mask_key(server_key), language=None)
            if st.button("이 키로 시작하기", type="primary", use_container_width=True):
                st.session_state.authenticated = True
                st.rerun()
            st.divider()
            input_label = "다른 키로 로그인하기"
        else:
            st.info("서버에 등록된 키가 없습니다. 본인의 OpenAI API 키를 입력해주세요.")
            input_label = "OpenAI API 키 등록"
...
def render_account_section():
    ...
    st.markdown("### 👤 계정")
    st.markdown(f"<span class='login-badge'>{source}</span>", unsafe_allow_html=True)
